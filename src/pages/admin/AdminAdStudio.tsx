...
export default function AdminAdStudio() {
  ...
  // Slice slots to a max of 12 so we don't overflow the absolute template design
            ...
            <div className="bg-card border border-border rounded-2xl p-6">
              ...
              {slots.length > 12 && (
                <p className="text-xs text-amber-500 mb-4 bg-amber-500/10 p-2 rounded-lg border border-amber-500/20">
                  Note: Found {slots.length} slots, but only the first 12 will visibly fit into the Instagram story template.
                </p>
              )}
            ...
            <div ref={captureRef} className="w-[1080px] h-[1920px] bg-black flex flex-col justify-between overflow-hidden relative">
                ...
                <div className="relative z-10 p-20 flex flex-col h-full">
                  ...
                  <div className="flex-1 flex flex-col justify-center items-center mt-20">
                    {displaySlots.length > 0 ? (
                      <div className="w-full">
                        <h2 className="text-[#10b981] text-[55px] font-black text-center mb-16 uppercase tracking-widest text-shadow-sm">Available Slots</h2>
                        <div className="grid grid-cols-2 gap-8 w-full max-w-4xl mx-auto">
                          {displaySlots.map(s => (
                            <div key={s} className="bg-white/5 backdrop-blur-md border-[3px] border-[#10b981]/50 text-white rounded-3xl py-8 px-6 text-center font-bold text-[45px] shadow-[0_0_30px_rgba(16,185,129,0.1)]">
                              {formatToAMPM(s)}
                            </div>
                          ))}
                        </div>
                  ...
                  <div className="mt-auto text-center mb-16 bg-black/40 backdrop-blur-md border border-white/10 rounded-[3rem] py-12 px-8">
                     ...
                     <p className="text-[#10b981] text-5xl font-black tracking-widest flex justify-center items-center gap-4">
                       ⚽ www.turfbook.pro
                     </p>
                  </div>
                </div>
