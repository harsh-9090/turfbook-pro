    ...
    <section className="py-20 lg:py-32 relative bg-card/50 overflow-hidden">
      ...
      >
        {displayItems.map((t, i) => (
          <div key={`${t.id}-${i}`}
            className={`snap-center flex-shrink-0 w-[320px] p-6 rounded-2xl bg-card border transition-all ${
              t.source === 'google' ? 'border-blue-500/20 hover:border-blue-500/40' : 'border-border hover:border-primary/20'
            }`}>
            ...
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-primary-foreground font-bold text-sm ${
                t.source === 'google' ? 'bg-blue-600' : 'bg-gradient-turf'
              }`}>
                {t.name[0]}
              </div>
