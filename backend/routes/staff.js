...
router.get('/', authMiddleware, adminOnly, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, name, email, phone, allowed_tabs, created_at FROM users WHERE role = 'staff' ORDER BY created_at DESC`
    );
...
router.post('/', authMiddleware, adminOnly, async (req, res) => {
  try {
    const { name, email, phone, password, allowed_tabs } = req.body;
    ...
    const result = await pool.query(
      `INSERT INTO users (name, email, phone, password_hash, role, allowed_tabs) VALUES ($1, $2, $3, $4, 'staff', $5) RETURNING id, name, email, phone, allowed_tabs, created_at`,
      [name, email, phone || '', passwordHash, allowed_tabs || []]
    );
